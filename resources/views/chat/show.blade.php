    ...
    <div class="bg-white shadow-lg rounded-lg flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50" id="messages-container">
            <template x-for="msg in messages" :key="msg.id">
                <div :class="msg.user_id == {{ auth()->id() }} ? 'chat-row align-end' : 'chat-row align-start'">
                    <!-- show avatar on left for others, on right for me -->
                    <template x-if="msg.user_id != {{ auth()->id() }}">
                        <div class="chat-avatar">
                            <img :src="msg.user.avatar_url || '{{ asset('assets/images/default-avatar.png') }}'" alt="avatar" style="width:100%;height:100%;object-fit:cover;">
                        </div>
                    </template>

                    <div :class="msg.user_id == {{ auth()->id() }} ? 'chat-bubble chat-bubble--me' : 'chat-bubble chat-bubble--other'">
    ...
    return {
        ...
        async fetchNewMessages() {
            try {
                ...
                const resp = await axios.get('{{ route('chat.messages.json', $room) }}');
        ...
        async deleteMessage(msg, action = 'me') {
            try {
                await axios.post('{{ route('chat.messages.delete', ['message' => '__MSG__']) }}'.replace('__MSG__', msg.id), { action });
                // remove locally for current user
                if (action === 'me' || (action === 'everyone' && msg.user_id == {{ auth()->id() }})) {
        ...
        async sendMessage() {
            ...
            try {
                ...
                const response = await axios.post('{{ route('chat.messages.store', $room) }}', form, {
...
(function(){
    ...
    const messagesContainer = document.getElementById('messages-container');
        ...
        e.preventDefault();
        const url = '{{ route('chat.messages.store', $room) }}';
        ...
        .then(async (res) => {
            ...
            if (res.ok && data) {
                const msg = data.id ? data : (data.message || null);
                    ...
                    wrapper.className = (msg.user_id == {{ auth()->id() }}) ? 'chat-row align-end' : 'chat-row align-start';
                    const bubbleClass = (msg.user_id == {{ auth()->id() }}) ? 'chat-bubble chat-bubble--me' : 'chat-bubble chat-bubble--other';
                    const text = msg.content || msg.message || '';
                    const iconHtml = (msg.user_id == {{ auth()->id() }}) ? '<span class="bubble-icon text-gray-700"><i class="fas fa-check"></i></span>' : '<span class="bubble-icon text-white"><i class="fas fa-user"></i></span>';

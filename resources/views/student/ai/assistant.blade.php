...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>مساعد همّه الذكي: {{ $course->title }}</h2>
        <div>
            <a href="{{ route('courses.show', $course) }}" class="btn btn-outline-secondary">العودة للكورس</a>
        </div>
    </div>
...
async function sendMessage(text){
    ...
    try{
        const res = await fetch("{{ route('student.courses.ai.ask', $course) }}",{
